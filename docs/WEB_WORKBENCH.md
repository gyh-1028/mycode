...
打开浏览器，也可以使用 `--port` 和 `--no-open`。使用 `--no-open` 时，终端会打印包含
临时令牌的完整地址，必须使用该地址首次打开工作台。
...
- 顶栏可在空闲时快速切换已保存模型；完整模型参数和 API Key 在设置面板维护。
- 左侧可筛选会话、懒加载文件树和搜索项目；Agent 运行时当前会话会锁定。
- 代码预览支持定位搜索结果、选择代码，并把整个文件或选区作为下一条消息的显式上下文。
- 右侧检查器分别展示模型/工具活动、逐文件 unified diff 和自动选择的上下文及原因。
- 任务结束后会刷新受影响的文件和 Diff；最近检查点可以直接撤销。
- 计划模式生成结果后，可通过“执行此计划”切回执行模式继续同一会话。
...
标准确认允许“仅本次”或“本轮允许操作”。本轮许可只记住完全相同的命令或文件操作，
不会扩展为同类操作的通配许可，并在当前 Agent Run 结束后清除。

## 安全边界

- 启动时生成 256 位随机令牌；令牌通过 URL fragment 交给前端后立即从地址栏删除，并只在
  当前浏览器标签页的 `sessionStorage` 中保留，以支持刷新重连；关闭标签页后不会长期保存。
